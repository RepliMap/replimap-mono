...
resource "aws_s3_bucket_policy" "ac-share-int-stage-policy" {
  ...
  policy = jsonencode({"Statement": [{"Action": "s3:*", "Effect": "Allow", "Principal": {"AWS": "arn:aws:iam::${var.aws_account_id}:user/leo"}, "Resource": ["arn:aws:s3:::ac-share-int-prod", "arn:aws:s3:::ac-share-int-prod/*"]}, {"Action": ["s3:Get*", "s3:List*"], "Effect": "Allow", "Principal": {"AWS": ["arn:aws:iam::${var.aws_account_id}:user/rafael", "arn:aws:iam::${var.aws_account_id}:user/david.lu", "arn:aws:iam::${var.aws_account_id}:user/isaac.santos", "arn:aws:iam::${var.aws_account_id}:user/matheus.mei"]}, "Resource": ["arn:aws:s3:::ac-share-int-prod", "arn:aws:s3:::ac-share-int-prod/*"]}, {"Action": "s3:*", "Condition": {"Bool": {"aws:SecureTransport": "false"}}, "Effect": "Deny", "Principal": "*", "Resource": ["arn:aws:s3:::ac-share-int-prod/*", "arn:aws:s3:::ac-share-int-prod"], "Sid": "DenyInsecureConnections"}], "Version": "2012-10-17"})
}
