...
For Team and Sovereign users, RepliMap provides a **Trust Center** audit mode
that records every AWS API call made during a scan and generates compliance
reports proving all operations were read-only.
...
replimap trust-center report --report-format json --output compliance.json
```
